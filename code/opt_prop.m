function [SINR, W_jn, alpha, beta, Pj, pk] = opt_prop(theta_AP, Hjnk, Gijn, ...
    Phiij, P, sigma2_r, sigma2_c, Nsub, XPD, Rc,alg)
% Joint optimization for ISAC with communication constraints
...
alpha = allocate_ap(Hjnk, Gijn, Phiij, theta_AP, P, sigma2_r, ...
    sigma2_c, Nsub, XPD, Rc, K, M);
...
if Pmin >= P
    % Cannot satisfy communication constraints, fallback to original method
    fprintf('Warning: Cannot satisfy comm constraints with enhanced allocation, using fallback\n');
end
...
for it = 2:maxIter
    ...
    % Update beamforming
    W_jn = update_W(He_nk, t_nk, c_nk, P, eta_in, gamma_in, alpha, beta, Rc, sigma2_c, ...
        u_in, Pj, Phiij, theta_AP, XPD, Gijn, Nsub);
